...
def clean_xml_fragment(xml_string: str) -> str:
    """Remove namespaces/prefixes to simplify ET parsing."""
    xml_string = re.sub(r'\s+xmlns(:[^=\s]+)?="[^"]*"', '', xml_string)
    xml_string = re.sub(r'(</?)[a-zA-Z0-9_\-]+:([a-zA-Z0-9_\-]+)', r'\1\2', xml_string)
    # Keep namespaced attributes by normalizing prefix away: nc:operation -> operation
...
def extract_log_segments(content: str) -> List[Dict[str, Any]]:
    """
    Split log into [timestamp];SEND/RECV;Nbytes headers + payload body.
    """
...
def parse_mplane_log(file_path: str) -> StateStore:
    ...
    for seg in segments:
        ctx = {**seg, **extract_rpc_context(seg["body"])}

        # parse user-plane-configuration blocks (edit-config/get/get-config/replies)
        up_blocks = find_xml_blocks("user-plane-configuration", seg["body"])
        for blk in up_blocks:
            parse_user_plane_configuration(blk, state, ctx)

        # parse processing-elements
        pe_blocks = find_xml_blocks("processing-elements", seg["body"])
        for blk in pe_blocks:
            parse_processing_elements(blk, state, ctx)

        # lightweight notifications
        parse_notifications(seg["body"], state, ctx)
