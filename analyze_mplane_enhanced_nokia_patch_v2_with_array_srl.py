...
def validate_state(state: StateStore):
    ...
    for l in state.links_tx.values():
        if l.get("low-level-tx-endpoint"):
            referenced_eps.add(l["low-level-tx-endpoint"])
        if l.get("tx-array-carrier"):
            referenced_cars_tx.add(l["tx-array-carrier"])
    for l in state.links_rx.values():
        if l.get("low-level-rx-endpoint"):
            referenced_eps.add(l["low-level-rx-endpoint"])
        if l.get("rx-array-carrier"):
            referenced_cars_rx.add(l["rx-array-carrier"])
...
def extract_endpoint_summary(ep: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "endpoint-source-tag": ep.get("_endpoint_tag"),
        ...
        "supported-reference-level": ep.get("supported-reference-level"),
        "_meta": ep.get("_meta", {}),
    }
...
def render_report(state: StateStore, show: str = "chain") -> str:
    lines: List[str] = []
    w = lines.append
    ...
    if show in ("all", "chain"):
        ...
        for c in chains:
            ...
            if ep:
                ...
                for k in [
                    "endpoint-source-tag", "array", "endpoint-type", "frame-structure", "cp-type", "cp-length", "cp-length-other",
                ...
                if ri:
                    if isinstance(ri, list):
                        w(f"      ├── restricted-interfaces[{len(ri)}]")
                ...
                if srl:
                    ...
                    w(f"      ├── supported-reference-level[{len(srl_list)}]")
                    for i, item in enumerate(srl_list, 1):
                        if isinstance(item, dict):
                            sid = item.get("id")
                            smin = item.get("min")
                            smax = item.get("max")
                            w(f"      │   ├── [{i}] id={fmt(sid)}, min={fmt(smin)}, max={fmt(smax)}")
                        else:
                            w(f"      │   ├── [{i}] {fmt(item)}")
...
def parse_args():
    ...
    p.add_argument("--show", default="chain",
